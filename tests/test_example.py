...
def test_lazypay_login(page, url, test_data_value):
    ...
    with allure.step(f"Enter mobile number: {test_data_value('TEST_MOBILE_NUMBER')}"):
        lazy_pay.enter_mobile_number(test_data_value('TEST_MOBILE_NUMBER'))
        allure.attach(page.screenshot(), "Mobile Number Entered", allure.attachment_type.PNG)
    
    with allure.step("Fetch OTP from API"):
        otp = lazy_pay.fetch_otp(test_data_value('TEST_MOBILE_NUMBER'))
...
def test_paylater_heading(page, url, test_data_value):
    with allure.step("Initialize LazyPay page"):
        lazy_pay = LazyPayPage(page)
    ...
    with allure.step("Validate the heading text"):
        expected_heading = test_data_value('PAYLATER_HEADING')
